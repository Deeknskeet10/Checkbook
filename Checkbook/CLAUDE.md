...
Platform solution(s) in this folder. It covers the repo layout, tooling, and
...
> **Repo layout note:** This `Checkbook/` folder is one of two self-contained
> Power Platform projects under the parent directory; the sibling `../LRC/`
> (Long Range Calendar) is a completely separate application with its own
> publisher, solution, and duplicated `devenv.nix`/`.config`/`.pac` tooling.
> They share only the parent git repository. Everything below is relative to
> this `Checkbook/` folder.
