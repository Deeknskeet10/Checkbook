...
{
    /// <summary>
    /// Shared rollup math for Requirement Funding totals derived from child
    /// Prioritizations.
    ///
    /// Both the PrioritizationRollupToRequirementFunding plugin (Prio
    /// Create/Update/Delete trigger) and the RealignmentProcessor call this
    /// helper. The rollup plugin guards on context.Depth &gt; 1 to avoid
    /// re-entry, so depth-1 actors that depend on a fresh RF.FundedAmount /
    /// RF.ValidatedAmount after a nested Prioritization update — most notably
    /// RealignmentProcessor — must invoke this helper directly. This mirrors
    /// the same pattern used by TDPCalculationHelper.RecalculateLOATDP.
    /// </summary>
    public static class PrioritizationRollupHelper
    {
        /// <summary>
        /// Recalculates FundedAmount and ValidatedAmount on the given
        /// Requirement Funding by summing its approved + active child
        /// Prioritizations. Writes the totals to the RF synchronously.
        /// </summary>
            ...
            ITracingService tracing)
        {
            ...
            tracing.Trace($"RF {rfId} rollup: Funded={fundedTotal}, Validated={validatedTotal}");
