...

{
  ...
  scripts = {
    ...
    # NOTE: step registration is done by hand in the Plugin Registration Tool —
    # there is no Register-Plugins.ps1 in this project. This script only pushes
    # an updated assembly to an already-registered plugin.
    pp-plugin-push.exec = ''
      set -euo pipefail
      ...
      dotnet tool run pac -- plugin push \
        --pluginId "$PLUGIN_ID" \
        --pluginFile "$PLUGIN_DIR/bin/Debug/net462/Checkbook_Plugins.dll"
    '';
  };
